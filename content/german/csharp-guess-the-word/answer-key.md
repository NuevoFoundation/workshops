```markdown
---
title: "Lösungsblatt - C# Errate das Wort"
date: 2020-03-28T10:24:17-07:00
draft: false
hidden: true
weight: 18
---

### Aktivität 1:

```c#
using System;
	
class MainClass {
    public static void Main(string[] args) {
	    Console.WriteLine("Hallo dort");
	    Console.WriteLine("Ich bin großartig");
    }
}
```

### Aktivität 2:

```c#
using System;
	
class MainClass {
    public static void Main(string[] args) {
        Console.WriteLine("Nuevo" + " " + "Foundation");
        Console.WriteLine("Nue" + "vo " + "Foundation");
        Console.WriteLine("Nuevo " + "Foun" + "dation");
	}
}
```

### Aktivität 3:

```c#
class MainClass {
	public static void Main(string[] args) {
        Console.WriteLine(2-19);
        Console.WriteLine((3+5)*6);
        Console.WriteLine((13+5*8)/(6-(3+7)));
	}
}
```

### Aktivität 4:

```c#
class MainClass {
	public static void Main(string[] args) {
		Console.WriteLine(54 < (10 + 32));
		Console.WriteLine((37 / 5) == 7 );
		Console.WriteLine("Hallo" + "Welt" == "Hallo Welt");
		Console.WriteLine(false == false);
		Console.WriteLine((11 - 2 < 10) && (7 + 3 > 10));
		Console.WriteLine((true || 3 < 1) && (false || 3 > 1));
	}
}
```

### Aktivität 5:

```c#
using System;

class MainClass {
	public static void Main(string[] args) {
        var comp = "Computer";
        var five = 5;	
		Console.WriteLine(comp);
		Console.WriteLine(five);
		Console.WriteLine(comp + comp);
		Console.WriteLine(five + five);
		Console.WriteLine(comp + comp + comp);
		Console.WriteLine(five + five + five);
	}
}
```

### Aktivität 6:

```c#
using System;

class MainClass {
    public static void Main(string[] args) {
        Console.WriteLine("Willkommen!"); 
        Console.WriteLine("Wie kann ich Ihnen heute helfen?"); 
        var input = Console.ReadLine(); 
        Console.WriteLine("Sie haben gefragt: " + input);
        Console.WriteLine("Ich weiß die Antwort darauf nicht! Auf Wiedersehen!");
    }
}
```
```