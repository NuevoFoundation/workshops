...
**這是應用程式的 CSS 代碼，將其複製並粘貼到文件** `"style.css"`

{{% notice info %}}
### CSS 的作用是什麼？
CSS（層疊樣式表，Cascading Style Sheets）控制您的應用程式**外觀**：例如顏色、形狀、尺寸和動畫。沒有 CSS，您的應用程式仍然可以正常運作，但它看起來會像普通的未經設計的文字。

以下每個部分都為您的哥斯大黎加應用程式中的不同組件進行設計。
{{% /notice %}}
