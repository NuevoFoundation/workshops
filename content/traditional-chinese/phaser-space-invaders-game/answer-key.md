...
```javascript
create(){
    this.player.body.setCollideWorldBounds(true); // 玩家不能超出屏幕
...
playerShoot(){
    ...
    if (Phaser.Input.Keyboard.JustDown(this.spacebar)) {
        if(this.player.active) {
    		...
    		this.projectiles.add(playerBullet); //加到群組
    	}
    }
}
