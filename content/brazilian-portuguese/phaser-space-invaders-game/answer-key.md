...
create(){
    ...
    // exclua seu código de plano de fundo anterior e crie-o novamente como um sprite de bloco
...
enemyShoot() {
	...
	for(var i = 0; i < this.enemies.getChildren().length; i++) { //passa por cada inimigo
		...
		if (randomEnemyShoot == 1) { //10% de chance de atirar
...
hurtPlayer(player, enemy) {
	...
	// exclua o cronômetro anterior e coloque-o nesta declaração if-else
	if (this.lives >= 1) {
...
create(){
	...
	// substitua a declaração de início do jogo que você tinha antes por esta
