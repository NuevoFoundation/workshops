...
## Echoing Wildcards
让我们将通配符 ```*``` 和 ```<star />``` 标签结合使用，让机器人在响应中包含用户的输入。  
<img src="../img/learn.png" alt="" width="20%" style="display: block; margin-left: 0;" />

1. 如果用户输入 "My name is [name]"，机器人应该回复 "Nice to meet you, [name]!"。
示例:  
输入 | 输出  
---|--------------  
...
2. 如果用户输入 "My favorite color is [color]"，机器人应该回复 "I also like [color]!"。
示例:  
输入 | 输出  
---|--------------  
...
3. 如果用户输入 "My favorite food is [food]"，机器人应该回复 "[food]! Yum!"。
示例:  
输入 | 输出  
---|--------------  
