...
- `track` (o pista): es el número de pista al que se aplica el efecto. Nota: Para aplicar un efecto a la pista maestra, usa 0 como valor para la pista. La pista maestra es donde todo el audio se reproduce colectivamente juntos. Esta es una manera fácil de aplicar un efecto a todos los sonidos que se están procesando.
...
1. En el explorador de API de la izquierda, desplázate hasta la API `setEffect`. 
2. Inserta una llamada a la función `setEffect` en el código mediante el icono de pegar.
3. Necesitamos reemplazar los parámetros con nuestros propios valores. Coloca un valor entero para `track`. Pongamos 2.
4. Para el resto de los parámetros del efecto, haz clic en **Open** junto a la API `setEffect` y verás un enlace a "Every Effect Explained in Detail". Haz clic en él para abrir esta sección en el lado derecho de tu ventana.

<img src="../img/screenshot-seteffect-open.png" height="400"/>

<img src="../img/screenshot-seteffect-explain.png" width="200"/>

5. Puede que notes que Earsketch tiene muchas variaciones diferentes de efectos. De forma predeterminada, deberías ver la sección 10.1 BANDPASS a la derecha. Por ahora, usaremos el DELAY de la sección 10.4 como `type` en nuestra llamada a la función `setEffect`.
6. Ahora que hemos elegido nuestro `type`, necesitamos un `parameter`. Navega a la sección 10.4 a la derecha para explorar los diversos parámetros soportados para `DELAY`. Aquí hay una foto de esa sección. Te recomendamos usar `DELAY_FEEDBACK`.
    ![](../img/screenshot-effect-parameter.png)

7. Para el parámetro final, introduce un valor válido. Puesto que hemos elegido `DELAY_FEEDBACK`, debemos especificar un número entre -120.0 y -1.0 (como se muestra en la imagen de arriba). Ejemplo: Usa -6.0 como valor de efecto para `DELAY_FEEDBACK`.
