...
We will start with some basic stuff - loading a spritesheet and animation in Scene 1 for the player's bullets:

![scene1_preload](../media/8/scene1_preload.png)

The location of the spritesheet is `assets/playerBullets.png` and the frame width and height is 32:
...
The playerShoot method will check if the spacebar has been pressed and fire a projectile if it has. "Firing a projectile", in this case, means creating a playerBullet object and adding it to the projectile group if the player is active (still alive).
...
Now, to test this, we can simply change the height value that the bullet self-destructs to one that is on the screen:

![player_bullet_update](../media/8/player_bullet_update.png)

The code is working correctly if the bullet disappears at the specific height. After we test that the code works, we can simply change this value to one that is above the screen.
