...
From the last section, we were able to find the coefficient and intercept of our linear equation using the sample dataset of 30 employees.
<h3>
\[
    Intercept = 25792.20
\]
...
But it begs the question, is salary related to the years of experience? Is the relationship between the two values strong enough to claim they are related? Are 30 employees enough? How can we make sure these are the correct values?

You need to recall that you found the values by working with a small sample that was drawn from a much larger number of employees. Since we have only one sample, there will be more uncertainty in the intercept and coefficient values.

What can you do to make sure the values are valid?
...
The previous example teaches us that, when you work with samples, the **intercept** and **coefficient** values are not necessarily the true ones. We know that the actual value must lie between 2 values but we don't know what they are! This is known as the confidence interval.
...
Focus on the **stderr** value in the table. In this case, is 409.40. What this means is that, for any sample set, the **coefficient** value will vary by 409.40.

# The Normal Distribution

Let's take a step back and ask, from the coefficients we can see in "Exercise 2" how likely is it that they will repeat when running the program for other samples?

Machine learning uses a lot of statistics to determine the values we have gathered. In the world of statistics,  there is a graph used to measure how likely a value will appear in the data. This is known as the "Normal Distribution" or "Bell Curve" and it looks something like this:
...
\]
\[
    CoefficientRange = 9449.96 \pm2*(StdError)
\]
\[
